...
class PhotosScreen extends StatelessWidget {
  ...
  PhotosScreen(this.messageIndex, this.attachmentIndex);

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<ConversationBloc, ConversationState>(
      builder: (_, state) {
        final messages = (state as ConversationData)?.currentItems ?? [];
        ...
        return Scaffold(
          appBar: AppBar(
            title: Text(
                'Фото от ' + _profilesService.getProfile(message?.fromId).name),
          ),
