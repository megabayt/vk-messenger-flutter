...
class StoreProduct {
  ...
  factory StoreProduct.fromJson(Map<String, dynamic> json) => StoreProduct(
        ...
        icon: json["icon"] == null
            ? null
            : List<Icon>.from(json["icon"].map((x) => Icon.fromJson(x))),
        previews: json["previews"] == null
            ? null
            : List<Icon>.from(json["previews"].map((x) => Icon.fromJson(x))),
        hasAnimation:
  ...
  Map<String, dynamic> toJson() => {
        ...
        "icon": icon == null
            ? null
            : List<dynamic>.from(icon.map((x) => x.toJson())),
...
final typeValues = EnumValues({"stickers": Type.STICKERS});
